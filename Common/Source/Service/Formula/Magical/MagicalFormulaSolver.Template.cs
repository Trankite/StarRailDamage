...
namespace UIDesign.Source.Service.Formula.Magical
{
    public partial class MagicalFormulaSolver
    {
        ...
        private abstract class MagicalFormulaAssignSymbol : MagicalFormulaSymbol
        {
            ...
            public override bool Verify(MagicalFormula formula, [NotNullWhen(false)] out string? message)
            {
                ...
                {
                    return false.Configure(message = LocalString.ServiceFormulaVerifyMisuseAssignSymbol.SafeFormat(Name));
                }
                if (formula.Ended.IsNull())
                {
                    return false.Configure(message = LocalString.ServiceFormulaVerifyMissingOperand.SafeFormat(Name));
                }
        ...
        private abstract class MagicalFormulaPrefixSymbol : MagicalFormulaSymbol
        {
            ...
            public override bool Verify(MagicalFormula formula, [NotNullWhen(false)] out string? message)
            {
                ...
                {
                    return false.Configure(message = LocalString.ServiceFormulaVerifyMisuseAffixeSymbol.SafeFormat(Name));
                }
                if (formula.Ended.IsNull())
                {
                    return false.Configure(message = LocalString.ServiceFormulaVerifyMissingOperand.SafeFormat(Name));
        ...
        private abstract class MagicalFormulaMethodSymbol : MagicalFormulaSymbol
        {
            ...
            {
                return context.Count < MinCount ? false.Configure(message = LocalString.ServiceFormulaVerifyMissingOperand.SafeFormat(Name)) : true.Configure(message = default);
            }
        }
    }
}