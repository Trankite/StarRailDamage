...
{
    public class DeviceFpResponseBuilderFactory : HoyolabHttpRequestMessageBuilderFactory
    {
        private const string URL = "https://public-data-api.mihoyo.com/device-fp/api/getFp";

        public DeviceFpResponseBuilderFactory() { }

        public DeviceFpResponseBuilderFactory(HoyolabToken hoyolabToken) : base(hoyolabToken) { }
        ...
        public override HttpRequestMessageBuilder Create()
        {
            ...
            return new HttpRequestMessageBuilder()
                ...
                .SetStringContent(JsonSerializer.Serialize(new
                {
                    device_id = Random.Shared.GetLowerHexString(16),
                    seed_id = Guid.NewGuid().ToString(),
                    seed_time = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds().ToString(),
                    platform = "2",
                    device_fp = string.IsNullOrEmpty(HoyolabToken.Device) ? Random.Shared.GetLowerHexString(13) : HoyolabToken.Device,
                    app_name = "bbs_cn",
                    ext_fields = JsonSerializer.Serialize(ExtendFields),
                    bbs_device_id = Guid.NewGuid().ToString()
                }));
        }
    }
}