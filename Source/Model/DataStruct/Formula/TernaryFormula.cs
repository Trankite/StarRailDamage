...
namespace StarRailDamage.Source.Model.DataStruct.Formula
{
    public class TernaryFormula<T>
    {
        ...
        public TernaryFormula<T> SetFormula(string formula) => this.Configure(Formula = FormulaSeparator.Parse(formula));

        public TernaryFormula<T> SetFormula(FormulaNode? formulaNode) => this.Configure(Formula = formulaNode);
        ...
        {
            if (formulaNode is null) return double.NaN;
        ...
        {
            return readOnly ? value : value.Configure(TrySetValue(formulaNode.Left?.Text, source, sender, value));
        }
        ...
        {
            return sender is not null && TryGetPropertyExpression(text, source, out IPropertyExpression<T, double>? PropertyExpression) && PropertyExpression.TryGetValue(sender, out result) || false.Configure(result = default);
        }
        ...
        {
            return text is not null && sender is not null && TryGetPropertyExpression(text, source, out IPropertyExpression<T, double>? PropertyExpression) && PropertyExpression.TrySetValue(sender, value);
        }
        ...
        {
            return Formula is null ? string.Empty : Formula.ToString();
        }
